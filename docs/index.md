...
| 机器人账号、服务账号和自动化主体的 API/CLI 边界 | [机器人账号与服务账号规划](bot-service-account-plan.md) |
...
机器人账号与服务账号仍处在规划阶段。Gitea 当前底层支持 bot 用户类型，本机 admin CLI 可创建 bot，但稳定 REST API 尚未完整暴露 bot 管理能力；第一版 ChatTea 设计见 [机器人账号与服务账号规划](bot-service-account-plan.md)。
