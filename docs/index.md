...
| 单域名/单端口生产约束下整合 Git、Pages、Control 的内部入口 | [本地运行时统一入口](local-runtime-entry.md) |
| Git、MySQL、Actions Runner、Pages 的 user-level 文件系统边界 | [运行时文件系统与服务边界](runtime-filesystem-layout.md) |
| Git service + Pages service 的独立服务方案、Actions 发布流和 URI 约定 | [Gitea Pages 机制与静态站点发布](gitea-pages.md) |
| GitHub-like Pages 分支部署：runner push `gh-pages`，Pages Host checkout/sync/serve | [Git-backed Pages 分支部署](gitea-pages-branch-deploy.md) |
| 从 SQLite 切换到 ChatData-managed MySQL 后端 | [Gitea MySQL 后端迁移](gitea-mysql-migration.md) |
| 内部账号、组织、团队、private/internal 可见性模型 | [账号、组织与可见性模型](account-organization-visibility.md) |
...
| 简明 CLI 能力地图与当前封装边界 | [CLI 能力地图](chattea-cli-tree.md) |
| 官方 `tea` CLI 与 ChatTea 覆盖范围对比 | [官方 tea CLI 对比](tea-cli-comparison.md) |
