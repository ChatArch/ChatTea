...
`chattea pages` 是当前 Git-backed Pages 更新任务的目标命令树，尚未进入正式发布的顶层 CLI；规划见本页“Pages 目标命令树”。
...
Pages 没有 Gitea 官方 REST 路由，ChatTea 需要把它实现为本机 runtime 能力。当前更新的推荐主路径是 Git-backed branch deploy：runner 只 push `gh-pages` / configured Pages branch，Pages Host 只 fetch/checkout/sync/serve。Host-local `publish` 只作为 smoke test 或兼容模式。

目标命令树：

```text
chattea pages
├── service                         # 管理 Pages static service
│   ├── bootstrap                   # 初始化 pages root/config 并安装 user service
│   ├── start                       # 启动 chattea-pages.service
│   ├── stop                        # 停止 chattea-pages.service
│   ├── restart                     # 重启 chattea-pages.service
│   ├── status                      # 查看 service 状态和监听地址
│   └── logs                        # 查看 Pages service 日志
├── config                          # 管理 repo-level Git Pages config
│   ├── set                         # 配置 source repo/branch/path/channel/trigger/credential ref
│   ├── view                        # 查看某个 repo 的 Git Pages config
│   ├── list                        # 列出配置了 Git Pages 的 repo
│   └── remove                      # 删除 repo 的 Git Pages config，不删除已发布站点
├── sync                            # 从配置好的 Git branch checkout/sync 到 pages/sites
│   ├── repo                        # 同步单个 repo
│   └── all                         # 同步所有 enabled repo；polling timer 可调用它
├── status                          # 查看 source branch、commit、last sync、URL 和 HTTP 状态
├── workflow                        # 输出 workflow 模板
│   ├── branch-template             # 推荐模板：runner build -> push gh-pages/pages branch
│   └── local-publish-template      # 兼容模板：runner 直接调用本机 publish
└── publish                         # 兼容模式：从本地静态目录直接写入 pages/sites
```

当前 Hitk 最小实践已经有 runtime prototype：`pagesctl.py config-set/list/sync/status`、`sync_branch.py` 和 `chattea-pages.service`。正式收进 CLI 时，应把 `pagesctl.py` 的 config/sync/status 语义提升为上面的 `chattea pages config/sync/status`，并保留 `publish` 作为 Host-local 兼容路径。

机制和实践证据见 [Git-backed Pages 分支部署](gitea-pages-branch-deploy.md)，旧 Host-local smoke test 和 URL 约定见 [Gitea Pages 机制与静态站点发布](gitea-pages.md)，文件边界见 [ChatTea 运行时文件系统与服务边界](runtime-filesystem-layout.md)。
...
- Pages 静态站点发布：当前 Gitea 实例没有官方 Pages REST 路由；当前更新方向是 ChatTea local runtime 提供 `pages service/config/sync/status/workflow`，推荐 Git-backed branch deploy，`publish` 仅作为 Host-local 兼容模式；默认 path URL 为 `<pages-domain>/<owner>/<repo>/`，resolver/custom domain 不在 v0.1；
