...
- Tightened the optional MySQL dependency to `ChatData>=0.1.1,<0.2.0` and rejected root+password MySQL backend configs that would not match the local ChatData root account.
- Added side-by-side migration support with custom Gitea service names and `server init --skip-gitea-migrate`, so a shadow instance can load the latest dump before nginx cutover.
